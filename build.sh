...
if [ -n "$BUILD" ]; then
  ...
  mkdir -p "build/$BUILD/root/etc/apt"
  cp -a /etc/apt/trusted.gpg* "build/$BUILD/root/etc/apt"
  ...
  if [ -n "$PATCH" ]; then
      ...
      git clone https://github.com/akelge/Volumio2.git build/$BUILD/root/volumio
  else
      git clone --depth 1 -b master --single-branch https://github.com/akelge/Volumio2.git build/$BUILD/root/volumio
  fi
  ...
  git clone --depth 1 -b dist --single-branch https://github.com/akelge/Volumio2-UI.git "build/$BUILD/root/volumio/http/www"
  echo 'Cloning Volumio Contemporary UI'
  git clone --depth 1 -b dist3 --single-branch https://github.com/akelge/Volumio2-UI.git "build/$BUILD/root/volumio/http/www3"
